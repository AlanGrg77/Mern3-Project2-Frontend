...
const Checkout = () => {
    ...
    const { khaltiUrl , error , status } = useAppSelector((state)=>state.checkout)
    ...
    const handleForm = async (e:FormEvent<HTMLFormElement>) =>{
      e.preventDefault()
        const productData = cartItems.length > 0 ? cartItems.map((c)=>(
          {
                ...
                cartItems.length > 0  ? cartItems.map((c)=>{
                    return (<div className="flex items-start gap-4">
                        <div className="w-32 h-28 max-lg:w-24 max-lg:h-24 flex p-3 shrink-0 bg-gray-200 rounded-md">
                          <img src={`http://localhost:3000/${c.product.productImageUrl}`} className="w-full object-contain" />
                        </div>
                        <div className="w-full">
                          <h3 className="text-sm lg:text-base text-gray-800">{c.product.productName}</h3>
                          <ul className="text-xs text-gray-800 space-y-1 mt-3">
                            <li className="flex flex-wrap gap-4">Quantity <span className="ml-auto">{c.quantity}</span></li>
                            <li className="flex flex-wrap gap-4">Total Price <span className="ml-auto">Rs. {c.product.productPrice}</span></li>
                          </ul>
                        </div>
                      </div> )
      ...
      <form onSubmit={handleForm} className="mt-8">
        ...
        <div className="mt-8">
          ...
          <div className="flex gap-1">
            ...
            <img onClick={()=>handlePayment(PaymentMethod.Khalti)} className={`w-[100px] h-[90px]  transition-all duration-200 ${
                    paymentMethodValue === PaymentMethod.Khalti ? "border-b-2 border-blue-500" : "hover:scale-125"
                }`} src={Khalti} alt="" />``
            <img onClick={()=>handlePayment(PaymentMethod.Esewa)} className={`w-[100px] h-[80px] mt-[10px]  transition-all duration-200 ${
                    paymentMethodValue === PaymentMethod.Esewa ? "border-b-2 border-blue-500" : "hover:scale-125"
                }`} src={Esewa} alt="" />
            <span onClick={()=>handlePayment(PaymentMethod.Cod)} className={`my-8 text-lg pb-[30px] transition-all duration-200 ${paymentMethodValue === PaymentMethod.Cod ? "border-b-2 border-blue-500" : "hover:text-2xl"}  cursor-pointer`}>Cash On Delivery</span>
          </div>
          {showError && error && <div className="text-red-500 mt-4">{error}</div>}
          <div className="flex gap-4 max-md:flex-col mt-8">
           <Link to="/cart"> <button type="button" className="rounded-md px-4 py-2.5 w-full text-sm tracking-wide bg-transparent hover:bg-gray-100 border border-gray-300 text-gray-800 max-md:order-1">Cancel</button></Link>
